#ifndef DEXAR_PE_FILE_H_
#define DEXAR_PE_FILE_H_
...
namespace dpr {
    ...
    constexpr uint8_t kPESignOffsetPtr = 0x3C;
    constexpr uint32_t kPESign = 0x00004550;    // PE\0\0
    constexpr uint32_t kCoffFileHeaderSize = 20;
    constexpr uint32_t kOptionalHeaderSize = 28;
    constexpr uint32_t kOptionalHeaderPlusSize = 24;
    constexpr uint32_t kOptionalHeaderWinSize = 68;
    constexpr uint32_t kOptionalHeaderWinPlusSize = 88;
    constexpr uint32_t kImageDataDirectorySize = 8;
    ...
    public:
        ...
        uint32_t getEpVirtualAddr() const;
        uint32_t getEpSecVirtualAddr() const;
        uint32_t getEpSecVirtualSize() const;

    private:
        bool processPE32(const uint8_t* buf, uint32_t coff_offset);
        bool processPE32Plus(const uint8_t* buf, uint32_t coff_offset);

        bool is_pe32_plus_;
        uint32_t ep_vir_addr_;
        uint32_t ep_offset_;

        uint32_t ep_sec_vir_addr_;
        uint32_t ep_sec_vir_size_;
    };

}

#endif  // DEXAR_PE_FILE_H_
#ifndef UTILS_BIG_INTEGER_BIG_INTEGER_H_
#define UTILS_BIG_INTEGER_BIG_INTEGER_H_

// 按照图书：BigNum Math: Implementing Cryptographic Multiple Precision Arithmetic
// 中的代码编写而成，书中代码来自 LibTomMath 库

#include "utils/big_integer/int_array.h"
...
namespace utl {
    ...
    public:
        ...
        static BigInteger fromString(const string8& str, int radix);
        // 未测试
        static BigInteger fromBytesBE(const stringu8& bytes);
        // 未测试
        static BigInteger fromBytesLE(const stringu8& bytes);
        ...
        void toString(int radix, string8* str) const;

        int getBitCount() const;
        int getByteCount() const;
        uint8_t getBit(uint32_t idx) const;
        // 未充分测试
        stringu8 getBytesBE() const;
        // 未充分测试
        stringu8 getBytesLE() const;
    ...
    private:
        ...
        static void readFromStringItl(const string8& str, int radix, IntArray* a);
        static void toStringItl(const IntArray& a, int radix, string8* str);
...
#endif  // UTILS_BIG_INTEGER_BIG_INTEGER_H_
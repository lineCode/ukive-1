...
namespace ukive {
    ...
    void OverlayScrollBar::update(int content_height, float percent) {
        ...
        int thumb_height = std::max(static_cast<int>(view_height * scale), scrollbar_min_width_);
    ...
    void OverlayScrollBar::setScrollBarMinWidth(int min_width) {
        if (min_width > 0) {
            scrollbar_min_width_ = min_width;
        }
    }

    void OverlayScrollBar::onDraw(Canvas* canvas) {
        if (!thumb_bounds_.empty()) {
            canvas->fillRect(std::move(thumb_bounds_.toRectF()), Color::Grey400);
        }
    }

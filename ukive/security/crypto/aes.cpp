...
namespace crypto {
    ...
    void AES::test() {
        //stringu8 key { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };
        //stringu8 key { 0x8e, 0x73, 0xb0, 0xf7, 0xda, 0x0e, 0x64, 0x52, 0xc8, 0x10, 0xf3, 0x2b, 0x80, 0x90, 0x79, 0xe5, 0x62, 0xf8, 0xea, 0xd2, 0x52, 0x2c, 0x6b, 0x7b };
        stringu8 key { 0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe, 0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81, 0x1f, 0x35, 0x2c, 0x07, 0x3b, 0x61, 0x08, 0xd7, 0x2d, 0x98, 0x10, 0xa3, 0x09, 0x14, 0xdf, 0xf4 };
        uint8_t input[] = { 0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d, 0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34 };

        uint8_t cipher[4 * Nb];
        encrypt(input, cipher, key);
    ...
    void AES::encrypt(const uint8_t in[4 * Nb], uint8_t out[4 * Nb], const stringu8& key) {
        uint32_t Nr = getNr(utl::STLCU32(key.length()) / 4, Nb);
    ...
    void AES::encrypt(const uint8_t in[4 * Nb], uint8_t out[4 * Nb], const std::vector<uint32_t>& w, uint32_t Nr) {
        ...
        addRoundKey(&context, w.data());

        for (uint32_t i = 1; i < Nr; ++i) {
            subBytes(&context);
            shiftRows(&context);
            mixColumns(&context);
            addRoundKey(&context, w.data() + i * Nb);
        }

        subBytes(&context);
        shiftRows(&context);
        addRoundKey(&context, w.data() + Nr * Nb);
    ...
    void AES::keyExpansion(const stringu8& key, std::vector<uint32_t>* out) {
        uint32_t Nk = utl::STLCU32(key.length()) / 4;
        uint32_t Nr = getNr(Nk, Nb);

        uint32_t i = 0;
        while (i < Nk) {
            out->at(i) = bytesToUInt32(key.data() + 4 * i);
            ++i;
        }
        ...
        while (i < Nb * (Nr + 1)) {
            uint32_t tmp = out->at(i - 1);
            if (i % Nk == 0) {
                tmp = subWord(rotWord(tmp)) ^ rcon(i / Nk);
            } else if (Nk > 6 && (i % Nk == 4)) {
                tmp = subWord(tmp);
            }
            out->at(i) = out->at(i - Nk) ^ tmp;
            ++i;
        }
    }

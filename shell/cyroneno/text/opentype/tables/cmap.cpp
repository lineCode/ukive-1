...
namespace otf {
    ...
    bool CMAP::parseTable(std::ifstream& file, uint32_t c, uint16_t* glyph_idx) {
        *glyph_idx = 0;
        int64_t start_offset = file.tellg();
    ...
    bool CMAP::parseFormat4(std::ifstream& file, uint16_t c, uint16_t* glyph_idx) {
        ...
        if (seg_count > 0) {
            ...
            for (uint16_t i = 0; i < seg_count; ++i) {
                auto tmp = ukive::swapBits(sub_tab.end_code[i]);
        ...
        if (seg_count > 0) {
            ...
            for (uint16_t i = 0; i < seg_count; ++i) {
                auto tmp = ukive::swapBits(sub_tab.start_code[i]);
        ...
        if (seg_count > 0) {
            ...
            for (uint16_t i = 0; i < seg_count; ++i) {
                auto tmp = ukive::swapBits(sub_tab.id_delta[i]);
        ...
        if (seg_count > 0) {
            ...
            for (uint16_t i = 0; i < seg_count; ++i) {
                auto tmp = ukive::swapBits(sub_tab.id_range_offset[i]);

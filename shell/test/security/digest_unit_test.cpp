...
namespace test {
    ...
    struct hash {
        const char *name;
        ukive::digest::SHAVersion whichSha;
        int hashsize;
        struct {
    ...
    { "SHA1", ukive::digest::SHAVersion::SHA1, ukive::digest::SHA1::kHashSize,
      {
    ...
    { "SHA224", ukive::digest::SHAVersion::SHA224, ukive::digest::SHA224::kHashSize,
      {
    ...
    { "SHA256", ukive::digest::SHAVersion::SHA256, ukive::digest::SHA256::kHashSize,
      {
    ...
    { "SHA384", ukive::digest::SHAVersion::SHA384, ukive::digest::SHA384::kHashSize,
      {
    ...
    { "SHA512", ukive::digest::SHAVersion::SHA512, ukive::digest::SHA512::kHashSize,
      {
        /* 1 */ { TEST1, length(TEST1), 1, 0, 0,
    ...
    struct hmachash {
      ...
      { /* 1 */ {
        ...
        "3A126854"
            ...
            ukive::digest::SHA1::kHashSize,
            ukive::digest::SHA224::kHashSize,
            ukive::digest::SHA256::kHashSize,
            ukive::digest::SHA384::kHashSize,
            ukive::digest::SHA512::kHashSize
        } },
      { /* 2 */ {
    ...
    struct hkdfhash {
        ukive::digest::SHAVersion which_sha;
        int ikmlength;
        const char *ikmarray;
        int saltlength;
        ...
        {   /* RFC 5869 A.1. Test Case 1 */
            ukive::digest::SHAVersion::SHA256,
...
void printResult(
    ...
    const char *hashname, const char *testtype, const string8& testname,
    const char *resultarray)
{
    if (resultarray) {
...
{
    ukive::digest::USHA sha;
    ukive::digest::HMAC hmac;
    ukive::digest::HKDF hkdf;
    int err, i;
    uint8_t Message_Digest_Buf[ukive::digest::USHA::kMaxHashSize];
    ...
    if (err != ukive::digest::shaSuccess) {
...
{
    unsigned char prk[ukive::digest::USHA::kMaxHashSize + 1];
    uint8_t okm[255 * ukive::digest::USHA::kMaxHashSize + 1];
    ...
    int err = ukive::digest::HKDF::calculate(
    ...
    printResult(okm, hkdfhashes[testno].okmlength,
        ukive::digest::USHA::USHAHashName(hkdfhashes[testno].which_sha), "hkdf standard test",
    ...
    err = ukive::digest::HKDF::hkdfExtract(hkdfhashes[testno].which_sha,
        reinterpret_cast<const uint8_t *>(hkdfhashes[testno].saltarray),
        hkdfhashes[testno].saltlength,
        reinterpret_cast<const uint8_t *>(hkdfhashes[testno].ikmarray),
    ...
    printResult(prk, ukive::digest::USHA::USHAHashSize(hkdfhashes[testno].which_sha),
        ukive::digest::USHA::USHAHashName(hkdfhashes[testno].which_sha), "hkdf standard test",
    ...
    err = ukive::digest::HKDF::hkdfExpand(hkdfhashes[testno].which_sha, prk,
        ukive::digest::USHA::USHAHashSize(hkdfhashes[testno].which_sha),
        reinterpret_cast<const uint8_t *>(hkdfhashes[testno].infoarray),
...
{
    int i;
    unsigned char SEED[ukive::digest::USHA::kMaxHashSize], MD[1003][ukive::digest::USHA::kMaxHashSize];
 ...
 */
void testErrors() {
    ukive::digest::USHA usha;
    uint8_t Message_Digest[ukive::digest::USHA::kMaxHashSize];
    int err = 0;

    for (int hashno = int(ukive::digest::SHAVersion::SHA1);
        hashno <= int(ukive::digest::SHAVersion::SHA512); ++hashno)
    {
        memset(&usha, '\343', sizeof(usha)); /* force bad data */
        usha.init(static_cast<ukive::digest::SHAVersion>(hashno));
        usha.result(Message_Digest);
        usha.update(reinterpret_cast<const uint8_t*>("foo"), 3);

        LOG(Log::INFO) << "\nError " << err << ". Should be " << ukive::digest::shaStateError << ".";
        LOG(Log::INFO) << hashes[hashno].name << " se: " <<
            ((err == ukive::digest::shaStateError) ? "PASSED" : "FAILED");
...
int TEST_SHA()
{
    ...
    {
        printf("Hash %s\n", hashes[hashno].name);
        int err = ukive::digest::shaSuccess;

        for (int loopno = 1; (loopno <= 5) && (err == ukive::digest::shaSuccess); ++loopno) {
            for (int testno = 0; (testno < TESTCOUNT) && (err == ukive::digest::shaSuccess); ++testno) {
                err = hash(
            ...
            for (int testno = 0; (testno < HMACTESTCOUNT) && (err == ukive::digest::shaSuccess); ++testno) {
            ...
            for (int testno = 0; (testno < HKDFTESTCOUNT) && (err == ukive::digest::shaSuccess); ++testno) {
...
int TEST_MD5() {
    DCHECK(ukive::digest::MD5::cal("") == "D41D8CD98F00B204E9800998ECF8427E");
    DCHECK(ukive::digest::MD5::cal("", 1) == "D41D8CD98F00B204E9800998ECF8427E");
    DCHECK(ukive::digest::MD5::cal("a") == "0CC175B9C0F1B6A831C399E269772661");
    DCHECK(ukive::digest::MD5::cal("a", 2) == "0CC175B9C0F1B6A831C399E269772661");
    DCHECK(ukive::digest::MD5::cal("12345678901234567890123456789012345678901234567890123456") == "49F193ADCE178490E34D1B3A4EC0064C");
    DCHECK(ukive::digest::MD5::cal("12345678901234567890123456789012345678901234567890123456", 1) == "49F193ADCE178490E34D1B3A4EC0064C");
    DCHECK(ukive::digest::MD5::cal("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789", 63) == "D174AB98D277D9F5A5611C2C9F419D9F");
    DCHECK(ukive::digest::MD5::cal("12345678901234567890123456789012345678901234567890123456789012345678901234567890") == "57EDF4A22BE3C955AC49DA2E2107B67A");
    DCHECK(ukive::digest::MD5::cal("12345678901234567890123456789012345678901234567890123456789012345678901234567890", 1) == "57EDF4A22BE3C955AC49DA2E2107B67A");
